defmodule EventDashboard.MetricsLive do
  use EventDashboard.Web, :live_view

  alias EventDashboard.ChartComponent
  ...
  def mount(params, %{"metrics" => {mod, fun}} = session, socket) do
    ...
    cond do
      ...
      group && is_nil(metrics) ->
        {:ok, push_redirect(socket, to: live_event_path(socket, :metrics, node()))}

      metrics && connected?(socket) ->
        EventDashboard.TelemetryListener.listen(socket.assigns.menu.node, metrics)
        {:ok, assign(socket, metrics: Enum.with_index(metrics))}

      first_group && is_nil(group) ->
        path = live_event_path(socket, :metrics, socket.assigns.menu.node, group: first_group)
          ...
          <%= for group <- @groups do %>
            <li class="nav-item">
              <%= live_redirect(format_group_name(group),
                    to: live_event_path(@socket, :metrics, @menu.node, group: group),
  ...
  def handle_info({:node_redirect, node}, socket) do
    ...
    {:noreply, push_redirect(socket, to: live_event_path(socket, :metrics, node, params))}
  end
end
