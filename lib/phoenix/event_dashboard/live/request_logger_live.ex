defmodule EventDashboard.RequestLoggerLive do
  ...
  def mount(%{"stream" => stream} = params, session, socket) do
    ...
    if connected?(socket) do
      ...
      Phoenix.PubSub.subscribe(pubsub_server, EventDashboard.RequestLogger.topic(stream))
    end
    ...
    if socket.assigns.menu.request_logger do
      ...
      {:ok,
       push_redirect(socket, to: live_event_path(socket, :home, socket.assigns.menu.node))}
    end
  end
  ...
  def mount(%{"node" => node}, %{"request_logger" => _}, socket) do
    ...
    {:ok,
     push_redirect(socket,
       to: live_event_path(socket, :request_logger, node, stream: stream)
     )}
  end
  ...
  def handle_info({:node_redirect, node}, socket) do
    to = live_event_path(socket, :request_logger, node, stream: socket.assigns.stream)
  ...
  def render(assigns) do
    ...
    <div class="row mb-3">
      <div class="col text-center">
        Want to refresh the logger parameter? <%= live_redirect "Start a new stream", to: live_event_path(@socket, :request_logger, @menu.node) %>
      </div>
    </div>
    """
  end
  ...
  defp sign(socket, key, value) do
    EventDashboard.RequestLogger.sign(socket.endpoint, key, value)
  end
