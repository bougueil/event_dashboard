defmodule EventDashboard.SocketInfoComponent do
  ...
  defp format_info(key, val, _live_event_path)
       when key in [:send_oct, :recv_oct],
       do: format_bytes(val)

  defp format_info(key, val, live_event_path)
       when key in [:connected, :port],
       do: format_value(val, live_event_path)
