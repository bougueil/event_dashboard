defmodule EventDashboard.LiveHelpers do
  @moduledoc false
  ...
  """
  def live_event_path(socket, page, node, params \\ []) do
    apply(
  ...
  Encodes PIDs for URLs.
  """
  def encode_event(id, at) when is_binary(id) do
    "Event #{id} #{at}"
  end
  ...
  Formats any value.
  """
  def format_value(port, live_event_path) when is_port(port) do
    live_patch(inspect(port), to: live_event_path.(node(port), info: encode_port(port)))
  end

  def format_value(pid, live_event_path) when is_pid(pid) do
    live_patch(inspect(pid), to: live_event_path.(node(pid), info: encode_pid(pid)))
  end

  def format_value([_ | _] = list, live_event_path) do
    {entries, left_over} = Enum.split(list, @format_limit)

    entries
    |> Enum.map(&format_value(&1, live_event_path))
  ...
  Formats ttl.
  """
  def format_ttl(ms) do
    cond do
      ms > (d = :timer.hours(24)) -> "#{div(ms, d)}d"
      ms > (d = :timer.hours(1)) -> "#{div(ms, d)}h"
      ms > (d = :timer.minutes(1)) -> "#{div(ms, d)}m"
      true -> "#{div(ms, 1000)}s"
    end
  end
  ...
  def live_modal(socket, component, opts) do
    ...
    live_component(socket, EventDashboard.ModalComponent, modal_opts)
  end
  ...
  def live_info(socket, %{info: {title, params}, node: node, page: page}) do
    if component = extract_info_component(title) do
      path = &live_event_path(socket, page, &1, Enum.into(&2, params))
  ...
  defp extract_info_component("PID<" <> _), do: EventDashboard.ProcessInfoComponent
  defp extract_info_component("Port<" <> _), do: EventDashboard.PortInfoComponent
  defp extract_info_component("Socket<" <> _), do: EventDashboard.SocketInfoComponent
  defp extract_info_component("ETS<" <> _), do: EventDashboard.EtsInfoComponent
  defp extract_info_component("Event " <> _), do: EventDashboard.EventInfoComponent
  ...
  def assign_mount(socket, page, params, session, refresher? \\ false) do
    ...
    capabilities = EventDashboard.SystemInfo.ensure_loaded(target_node)
    ...
    socket =
      Phoenix.LiveView.assign(socket, :menu, %{
        ...
        info: info(params),
        node: target_node,
        backend: capabilities.dashboard && session["backend"],
    ...
    if found_node do
      ...
      Phoenix.LiveView.push_redirect(socket, to: live_event_path(socket, :home, node()))
    end
  end
